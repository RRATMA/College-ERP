...
import { LogOut, ArrowLeft, CheckCircle, MapPin, BookOpen, ListChecks, UserPlus, Users, Link as LinkIcon, Clock, ShieldCheck, BarChart3, Info, Calendar } from 'lucide-react';
...
const styles = {
  ...
  loginCard: { background: 'white', padding: '45px', borderRadius: '35px', width: '100%', maxWidth: '400px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' },
  input: { padding: '14px', borderRadius: '12px', backgroundColor: '#f1f5f9', color: '#1e293b', border: '2px solid #e2e8f0', width: '100%', marginBottom: '15px', outline: 'none', fontSize: '15px' },
  ...
  rollBtn: (active) => ({ aspectRatio: '1/1', background: active ? '#10b981' : '#0f172a', color: 'white', borderRadius: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontWeight: '800', border: active ? 'none' : '1px solid #334155', transition: '0.2s' }),
  watermark: { position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', opacity: 0.04, width: '300px', pointerEvents: 'none' }
};
  ...
  if (view === 'login') return (
    <div style={{ ...styles.container, ...styles.centered, backgroundImage: 'url("/logo.png")', backgroundSize: '400px', backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundBlendMode: 'overlay' }}>
      <div style={styles.loginCard}>
        ...
        <h2 style={{ color: '#0f172a', fontSize: '16px', fontWeight: '900' }}>ATMA MALIK INSTITUTE</h2>
        <p style={{ color: '#2563eb', fontWeight: 'bold', marginBottom: '25px', fontSize: '12px' }}>AMRIT ERP PORTAL</p>
  ...
  return (
    <div style={styles.container}>
      <nav style={styles.nav}>
        <div><small style={{ color: '#94a3b8' }}>COMP DEPT</small><br /><b>{user.name}</b></div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', overflowX: 'auto' }}>
        {['Reports', 'Faculty', 'Link', 'Stats'].map((l, i) => (
          <button key={l} onClick={() => setTab(String(i+1))} style={{ padding: '12px', flex: 1, background: tab === String(i+1) ? '#2563eb' : '#1e293b', color: 'white', border: '1px solid #334155', borderRadius: '12px', cursor: 'pointer' }}>{l}</button>
        ))}
      </div>
      <div style={styles.card}>
        <img src="/logo.png" style={styles.watermark} alt="wm" />
        {tab === '1' && list.attendance.map(r => (
          <div key={r.id} style={{ padding: '15px', background: '#0f172a', borderRadius: '15px', marginBottom: '10px', display:'flex', justifyContent:'space-between', border: '1px solid #334155' }}>
        ...
        {tab === '2' && (
          <div>
            <input style={styles.input} placeholder="Faculty Name" onChange={e => setF({...f, name: e.target.value})} />
        ...
        {tab === '3' && (
          <div>
            ...
            <button style={styles.btnPrimary} onClick={async () => { await supabase.from('assignments').insert([{fac_id:f.sFac, class_name:f.sClass, subject_name:f.sSub}]); alert("Linked!"); refresh(); }}>LINK SUBJECT</button>
          </div>
        )}
      </div>
    </div>
...
function FacultyPanel({ user }) {
  ...
  const submitAtt = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      if (dist > 150) return alert(`❌ Range Error!\nYou are ${Math.round(dist)}m away. Attendance allowed only within 150m.`);
      ...
      // 3. Download Excel (All 6 Columns)
      const ws = XLSX.utils.json_to_sheet(students.map(s => ({ 
        "ROLL NO": s.id, "NAME": s.name, "STATUS": present.includes(s.id) ? "P" : "A",
        "SUBJECT": sel.sub, "DATE": timeStr, "TIME": `${sel.startTime}-${sel.endTime}`
      })));
      const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Attendance");
      XLSX.writeFile(wb, `${sel.class}_${sel.sub}.xlsx`);

      alert("✅ Attendance Submitted!"); setSel({ ...sel, sub: '' }); setPresent([]);
  ...
  if (!sel.sub) return (
    <div style={styles.card}>
      ...
      <select style={styles.input} onChange={e => setSel({...sel, type: e.target.value})}><option value="Lecture">Theory</option><option value="Practical">Practical</option></select>
  ...
  return (
    <div style={styles.card}>
      <div style={{display:'flex', justifyContent:'space-between', marginBottom:'20px'}}>
        <button onClick={() => setSel({...sel, sub:''})} style={{background:'none', border:'none', color:'#94a3b8', cursor:'pointer'}}><ArrowLeft/> Change</button>
        <div style={{textAlign:'right'}}><b>{sel.class} | {sel.sub}</b><br/><small>{sel.startTime}-{sel.endTime}</small></div>
      </div>
      
      <div style={{background:'#0f172a', padding:'15px', borderRadius:'15px', display:'flex', justifyContent:'space-around', marginBottom:'20px'}}>
        <div style={{textAlign:'center'}}><small>Present</small><br/><b style={{color:'#10b981'}}>{present.length}</b></div>
        <div style={{textAlign:'center'}}><small>Absent</small><br/><b style={{color:'#ef4444'}}>{students.length - present.length}</b></div>
      </div>
      ...
      <button style={{...styles.btnPrimary, marginTop:'30px', background:'#10b981'}} onClick={submitAtt}>SUBMIT & SYNC</button>
    </div>
  );
  }
