...
// --- GLOBAL STYLING (Tumchi Original Styling) ---
const injectStyles = () => {
    ...
    <div style={ui.container}>
      ...
      {tab === 'dashboard' && (
        <div className="dashboard-grid">
          ...
          <div className="glass-card stat-card-new">
            <div style={{display:'flex', justifyContent:'space-between'}}><TrendingUp size={22} color="#f59e0b"/><span style={{fontSize:'10px', color:'#f59e0b'}}>{attendancePercentage}%</span></div>
      ...
      {tab === 'defaulters' && (
        <div className="glass-card" style={{padding:'20px'}}>
           <p style={ui.label}>ABSENTEE RECORDS (DETAILED)</p>
           {db.abs.slice(0, 50).map((a, i) => (
             <div key={i} style={ui.feedRow}>
               <div><b>Roll: {a.student_roll}</b><br/><small>{a.class_name}</small></div>
               <div style={{color:'#f43f5e'}}>ABSENT</div>
             </div>
           ))}
        </div>
      )}
...
function FacultyPanel({ user, setView }) {
  ...
  const finalSync = async () => {
    ...
    try {
      ...
      // Auto Excel Download
      const ws = XLSX.utils.aoa_to_sheet([["PRESENT ROLLS"], [marked.join(", ")], [""], ["ABSENT ROLLS"], [students.filter(s=>!marked.includes(s.id)).map(s=>s.id).join(", ")]]);
      const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Attendance");
      XLSX.writeFile(wb, `${setup.cl}_Report.xlsx`);

      alert(testerMode ? "TESTER PASS" : "Synced!");
      setView('login');
  ...
  if (!active) return (
    <div style={ui.mobileWrap}>
      <div className="tester-badge" onClick={() => {setTesterMode(!testerMode); alert("Tester Mode Active");}}><Beaker size={20}/></div>
  ...
  return (
    <div style={ui.mobileWrap}>
      ...
      <button disabled={loading} onClick={submit} style={{...ui.submitBtn, background: testerMode?'#7c3aed':'#10b981'}}>{loading?"SYNCING...":testerMode?"TEST ALL PHASES":"SYNC TO SERVER"}</button>
    </div>
  );
}
