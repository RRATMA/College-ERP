...
import { 
  ...
  Monitor, Clock, CheckCircle2, AlertCircle, BarChart3, TrendingUp, ShieldAlert
} from 'lucide-react';
...
// --- GLOBAL STYLING ---
const injectStyles = () => {
  if (typeof document === 'undefined' || document.getElementById('amrit-v-final-pro')) return;
  ...
  styleTag.innerHTML = `
    ...
    .action-btn { background: #0891b2; color: #fff; border: none; padding: 15px; border-radius: 14px; font-weight: bold; cursor: pointer; display: flex; alignItems: center; justifyContent: center; gap: 10px; width: 100%; }
  `;
...
const INST_NAME = "ATMA MALIK INSTITUTE OF TECHNOLOGY AND RESEARCH";
  ...
  if (view === 'login') return (
    <div style={ui.loginWrap}>
      <div className="glass-card" style={ui.loginCard}>
        <div style={ui.logoCircle}><img src="/logo.png" alt="Logo" style={{width:'100%'}} /></div>
        ...
        <button onClick={() => handleLogin(document.getElementById('u').value, document.getElementById('p').value)} className="action-btn" style={{marginTop:'25px'}}>
    ...
    <div style={ui.container}>
      ...
      {tab === 'dashboard' && (
        <div className="dashboard-grid">
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(6,182,212,0.1)', color:'#06b6d4'}}><Database/></div><div><h2>{db.logs.length}</h2><p style={ui.dashLabel}>TOTAL LOGS</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(168,85,247,0.1)', color:'#a855f7'}}><Users/></div><div><h2>{db.facs.length}</h2><p style={ui.dashLabel}>STAFF</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(16,185,129,0.1)', color:'#10b981'}}><BookOpenCheck/></div><div><h2>{excelSheets.length}</h2><p style={ui.dashLabel}>CLASSES</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(236,72,153,0.1)', color:'#ec4899'}}><TrendingUp/></div><div><h2>{avg}%</h2><p style={ui.dashLabel}>AVG ATTENDANCE</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(14,165,233,0.1)', color:'#0ea5e9'}}><Zap/></div><div><h2>{todayLogs.length}</h2><p style={ui.dashLabel}>LOGS TODAY</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(244,63,94,0.1)', color:'#f43f5e'}}><ShieldAlert/></div><div><h2>{defs.length}</h2><p style={ui.dashLabel}>DEFAULTERS</p></div></div>
        </div>
      )}

      {tab === 'defaulters' && (
        <div className="glass-card" style={{padding:'20px'}}>
          <h4 style={{color:'#f43f5e', marginTop:0}}>Critical Defaulters (&ge; 5 Absents)</h4>
          {defs.map(d => (
            <div key={d.r} style={{display:'flex', justifyContent:'space-between', padding:'12px', borderBottom:'1px solid rgba(255,255,255,0.05)'}}>
      ...
      {tab === 'logs' && (
        <div>
          ...
          }} className="action-btn" style={{marginBottom:'15px'}}><Download size={18}/> DOWNLOAD ALL LOGS</button>
          {db.logs.map(log => (
      ...
      {tab === 'staff' && (
        <div>
          <div className="glass-card" style={{padding:'20px', marginBottom:'20px'}}>
            ...
            <button className="action-btn" style={{marginTop:'15px'}} onClick={async()=>{await supabase.from('faculties').insert([{id:form.id, name:form.name, password:form.pass}]); loadData(); alert("Staff Added");}}>REGISTER STAFF</button>
          </div>
          {db.facs.map(f => (
      ...
      {tab === 'mapping' && (
        <div className="glass-card" style={{padding:'20px'}}>
          ...
          <button className="action-btn" style={{marginTop:'20px', background:'#a855f7'}} onClick={async()=>{await supabase.from('assignments').insert([{fac_id:form.fId, class_name:form.cls, subject_name:form.sub}]); alert("Mapped");}}>SAVE MAPPING</button>
        </div>
      )}
    </div>
  );
...
function FacultyPanel({ user, setView }) {
  ...
  const submit = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      const reportData = [
        ...
        ["Faculty:", user.name, "Type:", setup.ty],
        ["Time:", `${setup.start} to ${setup.end}`],
        [],
        ["Roll No", "Name", "Status"]
      ];
      students.forEach(s => reportData.push([s.id, s.name, marked.includes(s.id) ? "PRESENT" : "ABSENT"]));
      
      const ws = XLSX.utils.aoa_to_sheet(reportData);
      const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Sheet1");
      XLSX.writeFile(wb, `Attendance_${setup.cl}_${setup.sub}.xlsx`);

      alert("Sync Complete & Sheet Downloaded"); setView('login');
    }, () => { setLoading(false); alert("GPS Required"); });
  };

  if (!active) return (
    <div style={ui.mobileWrap}>
      <div style={ui.header}><div><small>Faculty Portal</small><h4>{user.name}</h4></div><button onClick={()=>setView('login')} style={ui.exitBtn}><LogOut/></button></div>
      ...
      {setup.cl && (
        <div style={{marginTop: '20px'}}>
          <select style={{marginBottom:'10px'}} onChange={e=>setSetup({...setup, sub:e.target.value})}><option>Select Subject</option>{myJobs.filter(j=>j.class_name===setup.cl).map(j=><option key={j.id} value={j.subject_name}>{j.subject_name}</option>)}</select>
          ...
          <button onClick={launch} className="action-btn" style={{marginTop:'25px'}}><Zap size={18}/> START SESSION</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={ui.mobileWrap}>
      ...
      <button disabled={loading} onClick={submit} style={ui.submitBtn}>{loading ? "VERIFYING GPS..." : "FINALIZE & DOWNLOAD SHEET"}</button>
    </div>
  );
}
