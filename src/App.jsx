...
function HODPanel({ excelSheets, setView }) {
  ...
  const downloadDefaulters = () => {
    // Grouping by roll number and counting absences
    ...
    const defData = Object.entries(defCounts).map(([roll, count]) => ({
      ...
      "Status": count > 5 ? "Critical Defaulter" : "Needs Warning"
    ...
    XLSX.writeFile(wb, "Amrit_Defaulter_Report.xlsx");
  };
      ...
      {tab === 'logs' && (
        <div>
          <div style={{display:'flex', gap:'10px', marginBottom:'20px'}}>
            ...
            <button title="Download Attendance" style={{background:'#10b981', padding:'0 15px', color:'#fff', borderRadius:'12px', border:'none'}} onClick={downloadAttendance}><Download size={20}/></button>
            <button title="Download Defaulters" style={{background:'#f59e0b', padding:'0 15px', color:'#fff', borderRadius:'12px', border:'none'}} onClick={downloadDefaulters}><FileDown size={20}/></button>
          </div>
...
const ui = {
  ...
  label: { fontSize: '10px', color: '#64748b', fontWeight: '
