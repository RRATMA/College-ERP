...
function HODPanel({ excelSheets, setView }) {
  ...
  const todayTheory = todayLogs.filter(l => l.type === 'Theory').length;
  const todayPractical = todayLogs.filter(l => l.type === 'Practical').length;
      ...
      {tab === 'dashboard' && (
        <div className="dashboard-grid">
          ...
          <div className="glass-card stat-card-new">
            ...
            <div><h2 style={{margin:0}}>{totalPresent}P / {totalAbsent}A</h2><p style={ui.dashLabel}>STUDENT COUNT</p></div>
          </div>
          <div className="glass-card stat-card-new">
          ...
          <div className="glass-card stat-card-new">
            ...
            <div><h2 style={{margin:0}}>{todayTheory}T | {todayPractical}P</h2><p style={ui.dashLabel}>LOAD TYPE</p></div>
          </div>
        </div>
      )}
      ...
      {tab === 'staff' && (
        <div>
          <div className="glass-card" style={{padding:'20px', marginBottom:'20px'}}>
            <p style={ui.label}>NEW REGISTRATION</p>
...
function FacultyPanel({ user, setView }) {
  ...
  if (!active) return (
    <div style={ui.mobileWrap}>
      <div style={ui.header}><div><small>Prof.</small><h4>{user.name}</h4></div><button onClick={()=>setView('login')} style={ui.exitBtn}><LogOut/></button></div>
