...
// --- HOD PANEL (With Staff Analytics & Master Search) ---
function HODPanel({ excelSheets, setView }) {
  ...
  return (
    <div style={ui.container}>
      ...
      {tab === 'staff' && (
        <div>
          <p style={ui.label}>STAFF PERFORMANCE (Theory/Practical Counts)</p>
          {db.facs.map(f => {
            ...
            return (
              <div key={f.id} className="glass-card" style={ui.feedRow}>
                ...
                <div style={{textAlign:'right'}}>
                   <span style={{color:'#06b6d4'}}>T: {theory}</span> | <span style={{color:'#10b981'}}>P: {practical}</span>
                </div>
              </div>
            );
          })}
      ...
      {tab === 'logs' && (
        <div>
          <div style={{display:'flex', gap:'10px', marginBottom:'15px'}}>
            <input placeholder="Search Master Sheet (Class, Subject, Faculty)..." onChange={e=>setSearch(e.target.value)} />
            <button onClick={downloadMaster} style={{background:'#10b981', color:'#fff', padding:'10px', borderRadius:'12px', border:'none'}}><Download/></button>
          </div>
      ...
      {/* ... Dashboard & Mapping remains same as previous UI ... */}
    </div>
  );
}

// --- FACULTY PANEL (With Time & Type Features) ---
function FacultyPanel({ user, setView }) {
  ...
  const startSession = () => {
    if(!setup.cl || !setup.sub || !setup.start) return alert("Fill Time & Type!");
  ...
  const submit = () => {
    if(tester) return sync();
    navigator.geolocation.getCurrentPosition(sync, () => alert("GPS Error"));
  };
  ...
  if (!active) return (
    <div style={ui.mobileWrap}>
      ...
      <div style={{display:'flex', gap:'10px', marginTop:'10px'}}>
        <button onClick={()=>setSetup({...setup, ty:'Theory'})} style={{...ui.typeBtn, background: setup.ty==='Theory'?'#0891b2':'#1e293b'}}>Theory</button>
        <button onClick={()=>setSetup({...setup, ty:'Practical'})} style={{...ui.typeBtn, background: setup.ty==='Practical'?'#10b981':'#1e293b'}}>Practical</button>
      </div>

      <button onClick={startSession} style={{...ui.primaryBtn, marginTop:'20px'}}><Zap/> START</button>
    </div>
  );
  ...
  return (
    <div style={ui.mobileWrap}>
      ...
      <button onClick={submit} style={{...ui.submitBtn, background: tester?'#7c3aed':'#10b981'}}>SYNC TO SERVER</button>
    </div>
  );
}

// --- UI CONFIG (Same as yours) ---
const ui = {
  ...
  tabBtn: { padding: '10px 18px', borderRadius: '14px', border: 'none', color: '#fff', cursor: 'pointer' },
