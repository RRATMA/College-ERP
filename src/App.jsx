    ...
    <div style={ui.container}>
      <div style={ui.header}>
        <div style={{display:'flex', alignItems:'center', gap:'12px'}}>
          <img src="/logo.png" style={{width:'40px', borderRadius:'50%'}} /> <h3>HOD Panel</h3>
        </div>
      ...
      <div className="scroll-hide" style={ui.tabRow}>
        {['dashboard', 'staff', 'mapping', 'logs'].map(t => (
          <button key={t} onClick={()=>setTab(t)} style={{...ui.tabBtn, background: tab===t?'#0891b2':'#1e293b', display:'flex', alignItems:'center', gap:'8px'}}>
      ...
      {tab === 'dashboard' && (
        <div style={ui.statsGrid}>
          ...
          <div className="glass-card stat-card-new" style={{ color: '#a855f7' }}>
            ...
            <h2>{db.facs.length}</h2><p>STAFF</p>
          </div>
        ...
        <div>
          <div style={{display:'flex', gap:'10px', marginBottom:'15px'}}>
            <div style={{position:'relative', flex:1}}>
              ...
              <input placeholder="Search records..." style={{...ui.input, paddingLeft:'40px', marginBottom:0}} onChange={(e)=>setSearchTerm(e.target.value)} />
            </div>
            <button onClick={()=>{
                const ws = XLSX.utils.json_to_sheet(db.logs);
                const wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, "Attendance");
                XLSX.writeFile(wb, "Amrit_Logs.xlsx");
            }} style={{background:'#10b981', color:'#fff', border:'none', borderRadius:'12px', padding:'0 15px'}}><Download size={20}/></button>
          </div>
      ...
      {/* Staff & Mapping kept same as previous logic */}
      {tab === 'staff' && (
        <div>
      ...
      {tab === 'mapping' && (
        <div className="glass-card" style={{padding:'20px'}}>
           <p style={ui.label}><Layers size={12}/> ASSIGN LOAD</p>
           ...
           <button style={ui.primaryBtn} onClick={async()=>{await supabase.from('assignments').insert([{fac_id:form.fId, class_name:form.cls, subject_name:form.sub}]); loadData(); alert("Done!");}}>MAP SUBJECT</button>
        </div>
      )}
    </div>
  );
...
// --- FACULTY PANEL (ALL FEATURES INTACT) ---
function FacultyPanel({ user, setView }) {
  ...
  const launch = () => {
    if(!setup.cl || !setup.sub || !setup.start || !setup.end) return alert("Fill all details with Time!");
  ...
  if (!active) return (
    <div style={ui.mobileWrap}>
      <div style={ui.header}><div><small>Hello,</small><h4>Prof. {user.name}</h4></div><button onClick={()=>setView('login')} style={ui.exitBtn}><LogOut/></button></div>
      
      <p style={ui.label}><LayoutGrid size={12}/> CLASS</p>
      ...
      {setup.cl && (
        <div style={{marginTop: '20px'}}>
          ...
          <p style={ui.label}><Clock size={12}/> TIME</p>
          <div style={{display:'flex', gap:'10px', marginBottom:'15px'}}>
          ...
          <button onClick={launch} style={ui.primaryBtn}><Zap size={18}/> START</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={ui.mobileWrap}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'8px', paddingBottom:'80px'}}>{students.map(s => (<div key={s.id} onClick={() => setMarked(p=>p.includes(s.id)?p.filter(x=>x!==s.id):[...p, s.id])} style={{...ui.rollChip, background: marked.includes(s.id)?'#10b981':'#1e293b'}}>{s.id}</div>))}</div>
      <button onClick={submit} style={ui.submitBtn}>SYNC NOW</button>
    </div>
  );
}
...
const ui = {
  ...
  logoBox: { width: '80px', height: '80px', margin: '0 auto 20px' },
  container: { maxWidth: '800px', margin: '0 auto', padding: '20px' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' },
  tabRow: { display: 'flex', gap: '8px', marginBottom: '20px', overflowX: 'auto' },
  tabBtn: { padding: '10px 15px', borderRadius: '12px', border: 'none', fontWeight: 'bold', fontSize: '11px', color:'#fff' },
  ...
  tile: { padding: '20px 10px', borderRadius: '15px', textAlign: 'center', fontWeight: 'bold', color:'#fff' },
  subList: { display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '15px' },
  subRow: { padding: '12px', borderRadius: '10px', textAlign: 'center', fontWeight: 'bold', color:'#fff' },
