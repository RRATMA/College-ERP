...
const injectStyles = () => {
  ...
  styleTag.innerHTML = `
    ...
    .defaulter-card { grid-column: span 2; border: 1px solid rgba(244, 63, 94, 0.4) !important; background: linear-gradient(135deg, rgba(244, 63, 94, 0.15), rgba(30, 41, 59, 0.4)) !important; }
...
// Config: Campus Coordinates
const CAMPUS_LAT = 19.7042; 
const CAMPUS_LON = 72.7645;
const RADIUS_LIMIT = 0.0008; // ~100 Meters
...
export default function AmritApp() {
  ...
  useEffect(() => {
    ...
    }).catch(() => console.error("Excel File Not Found"));
  }, []);
  ...
  if (view === 'login') return (
    <div style={ui.loginWrap}>
      <div className="glass-card" style={ui.loginCard}>
        ...
        <input id="u" placeholder="Faculty ID" style={{marginBottom:'10px'}} />
...
function HODPanel({ excelSheets, setView }) {
  ...
  const downloadDefaulters = async () => {
    ...
    const defaulters = [];
    const uniqueStudents = [...new Set(absRecords.map(a => `${a.class_name}_${a.student_roll}`))];
    ...
    uniqueStudents.forEach(key => {
      ...
      if (parseFloat(perc) < 75) {
        defaulters.push({ "Class": cls, "Roll No": roll, "Total": totalLec, "Present": preCount, "Absent": absCount, "Percentage": perc + "%" });
      }
    });
    ...
    XLSX.writeFile(wb, `Defaulter_Report_75_${todayStr}.xlsx`);
  };
    ...
    <div style={ui.container}>
      <div style={ui.header}>
        <div style={{display:'flex', alignItems:'center', gap:'12px'}}>
          <img src="/logo.png" style={{width:'40px', height:'40px', borderRadius:'50%'}} />
      ...
      {tab === 'dashboard' && (
        <div className="dashboard-grid">
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(6,182,212,0.1)', color:'#06b6d4'}}><Database/></div><div><h2 style={{margin:0}}>{db.logs.length}</h2><p style={ui.dashLabel}>LOG ENTRIES</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(168,85,247,0.1)', color:'#a855f7'}}><UserCheck/></div><div><h2 style={{margin:0}}>{db.facs.length}</h2><p style={ui.dashLabel}>FACULTY</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(16,185,129,0.1)', color:'#10b981'}}><BookOpenCheck/></div><div><h2 style={{margin:0}}>{excelSheets.length}</h2><p style={ui.dashLabel}>CLASSES</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(245,158,11,0.1)', color:'#f59e0b'}}><TrendingUp/></div><div><h2 style={{margin:0}}>{attendancePercentage}%</h2><p style={ui.dashLabel}>AVG ATTENDANCE</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(236,72,153,0.1)', color:'#ec4899'}}><Zap/></div><div><h2 style={{margin:0}}>{todayLogs.length}</h2><p style={ui.dashLabel}>LOGS TODAY</p></div></div>
          <div className="glass-card stat-card-new"><div className="icon-box" style={{background:'rgba(99,102,241,0.1)', color:'#6366f1'}}><BarChart3/></div><div><h2 style={{margin:0}}>{todayLogs.filter(l=>l.type==='Theory').length}T | {todayLogs.filter(l=>l.type==='Practical').length}P</h2><p style={ui.dashLabel}>TYPE SPLIT</p></div></div>
          
          <div className="glass-card stat-card-new defaulter-card" style={{flexDirection:'row', alignItems:'center', gap:'20px', cursor:'pointer'}} onClick={downloadDefaulters}>
             <div className="icon-box" style={{background:'#f43f5e', color:'#fff', marginBottom:0, minWidth:'50px', minHeight:'50px'}}><FileWarning size={28}/></div>
             <div>
               <h3 style={{margin:0, color:'#f43f5e'}}>DOWNLOAD DEFAULTER LIST</h3>
               <p style={{margin:0, fontSize:'12px', color:'#64748b'}}>Generates Excel for students below 75% attendance</p>
             </div>
          </div>
        </div>
      )}
      ...
      {tab === 'staff' && (
        <div>
          <div className="glass-card" style={{padding:'25px', marginBottom:'20px'}}>
            <p style={ui.label}>CREATE FACULTY ACCOUNT</p>
            ...
            <button style={{...ui.primaryBtn, marginTop:'15px'}} onClick={async()=>{await supabase.from('faculties').insert([{id:form.id, name:form.name, password:form.pass}]); loadData();}}>ADD STAFF</button>
          </div>
          {db.facs.map(f => {
            const sLogs = db.logs.filter(l => l.faculty === f.name);
            return (
              <div key={f.id} style={ui.feedRow} className="glass-card">
                <div><b>{f.name}</b><br/><small>ID: {f.id} | Theory: {sLogs.filter(x=>x.type==='Theory').length} | Practical: {sLogs.filter(x=>x.type==='Practical').length}</small></div>
      ...
      {tab === 'logs' && (
        <div>
          <input placeholder="Search records..." style={{marginBottom:'15px'}} onChange={e=>setSearchTerm(e.target.value)}/>
...
function FacultyPanel({ user, setView }) {
  ...
  const submit = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      if(dist > RADIUS_LIMIT) { setLoading(false); return alert("Out of Campus Radius. Move closer to sync."); }
  ...
  if (!active) return (
    <div style={ui.mobileWrap}>
      ...
      <p style={ui.label}>CLASS SELECTION</p>
      ...
      {setup.cl && (
        <div style={{marginTop: '20px'}}>
          <p style={ui.label}>SUBJECT SELECTION</p>
  ...
  return (
    <div style={ui.mobileWrap}>
      ...
      <button disabled={loading} onClick={submit} style={ui.submitBtn}>{loading ? "WAITING FOR GPS..." : "SYNC TO SERVER"}</button>
    </div>
  );
}
...
const ui = {
  ...
  submitBtn: { position: 'fixed', bottom: '20px', left: '20px', right: '20px', padding: '18px', borderRadius: '18px', background: '#10b981
