...
const injectStyles = () => {
  ...
  styleTag.innerHTML = `
    ...
    .tile { padding: 25px 10px; border-radius: 20px; text-align: center; font-weight: 800; cursor: pointer; border: 2px solid transparent; }
  ...
  if (view === 'login') return (
    <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'radial-gradient(circle at top right, #083344, #020617)' }}>
      <div className="glass" style={{ padding: '40px', width: '300px', textAlign: 'center' }}>
        <div style={{ width: '80px', height: '80px', margin: '0 auto 20px' }}><img src="/logo.png" style={{width:'100%'}} alt="Logo" /></div>
        ...
        <button onClick={() => handleLogin(document.getElementById('u').value, document.getElementById('p').value)} style={{ width: '100%', padding: '15px', background: '#0891b2', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: 'bold', marginTop: '20px', cursor: 'pointer' }}>LOGIN</button>
      </div>
    </div>
  );
...
// --- HOD PANEL (6 CARDS + LOGOUT + DEFAULTERS) ---
    ...
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '20px' }}>
      ...
      {tab === 'dashboard' && (
        <div className="dashboard-grid">
          <div className="glass stat-card"><div className="icon-box" style={{background:'rgba(6,182,212,0.1)', color:'#06b6d4'}}><Database/></div><div><h2>{db.logs.length}</h2><p>TOTAL LOGS</p></div></div>
          <div className="glass stat-card"><div className="icon-box" style={{background:'rgba(168,85,247,0.1)', color:'#a855f7'}}><Users/></div><div><h2>{db.facs.length}</h2><p>STAFF</p></div></div>
          <div className="glass stat-card"><div className="icon-box" style={{background:'rgba(16,185,129,0.1)', color:'#10b981'}}><BookOpenCheck/></div><div><h2>{excelSheets.length}</h2><p>CLASSES</p></div></div>
          <div className="glass stat-card"><div className="icon-box" style={{background:'rgba(236,72,153,0.1)', color:'#ec4899'}}><TrendingUp/></div><div><h2>{avg}%</h2><p>AVG ATTENDANCE</p></div></div>
          <div className="glass stat-card"><div className="icon-box" style={{background:'rgba(244,63,94,0.1)', color:'#f43f5e'}}><Zap/></div><div><h2>{tLogs.length}</h2><p>TODAY</p></div></div>
          <div className="glass stat-card"><div className="icon-box" style={{background:'rgba(99,102,241,0.1)', color:'#6366f1'}}><ShieldAlert/></div><div><h2>{defs.length}</h2><p>DEFAULTERS</p></div></div>
        </div>
      )}

      {tab === 'defaulters' && (
        <div>
          <p style={{fontSize:'12px', color:'#f43f5e', fontWeight:'bold'}}>CRITICAL LIST (&ge; 5 ABSENTS)</p>
          {defs.map(d => (
            <div key={d.r} className="glass" style={{ display: 'flex', justifyContent: 'space-between', padding: '15px', marginBottom: '10px' }}>
              <span>Roll No: {d.r}</span>
              <span style={{color:'#f43f5e'}}>{d.c} Absents</span>
            </div>
          ))}
        </div>
      )}
      ...
      {tab === 'staff' && (
        <div>
          <div className="glass" style={{padding:'20px', marginBottom:'20px'}}>
            ...
            <button style={{ width: '100%', padding: '15px', background: '#0891b2', color: '#fff', border: 'none', borderRadius: '10px', marginTop: '15px' }} onClick={async()=>{await supabase.from('faculties').insert([{id:form.id, name:form.name, password:form.pass}]); loadAll(); alert("Saved");}}>ADD FACULTY</button>
          </div>
          {db.facs.map(f => (
            <div key={f.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '15px', marginBottom: '10px' }} className="glass">
              <div><b>{f.name}</b><br/><small>{f.id}</small></div>
      ...
      {tab === 'mapping' && (
        <div className="glass" style={{padding:'20px'}}>
          ...
          <select style={{marginTop:'10px'}} onChange={e=>setForm({...form, cls:e.target.value})}><option>Class</option>{excelSheets.map(s=><option key={s} value={s}>{s}</option>)}</select>
          <input placeholder="Subject" style={{marginTop:'10px'}} onChange={e=>setForm({...form, sub:e.target.value})}/>
          <button style={{ width: '100%', padding: '15px', background: '#a855f7', color: '#fff', border: 'none', borderRadius: '10px', marginTop: '15px' }} onClick={async()=>{await supabase.from('assignments').insert([{fac_id:form.fId, class_name:form.cls, subject_name:form.sub}]); loadAll(); alert("Mapped");}}>SAVE MAPPING</button>
        </div>
      )}
...
// --- FACULTY PANEL (TYPE SELECTION + TIME + LOGOUT) ---
function FacultyPanel({ user, setView }) {
  ...
  const submitAtt = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      // EXCEL DOWNLOAD
      const data = [["ATMA MALIK INSTITUTE OF TECHNOLOGY AND RESEARCH"], ["Class:", setup.cl, "Subject:", setup.sub], ["Faculty:", user.name], ["Type:", setup.ty, "Time:", `${setup.start}-${setup.end}`], ["Roll No", "Name", "Status"]];
      students.forEach(s => data.push([s.id, s.name, marked.includes(s.id) ? "PRESENT" : "ABSENT"]));
      const ws = XLSX.utils.aoa_to_sheet(data);
      const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Attendance");
      XLSX.writeFile(wb, `Attendance_${setup.cl}_${tStr}.xlsx`);
    ...
    <div style={{padding:'20px'}}>
      ...
      {setup.cl && (
        <div style={{marginTop:'20px'}}>
          <select onChange={e=>setSetup({...setup, sub:e.target.value})}><option>Subject</option>{myJobs.filter(j=>j.class_name===setup.cl).map(j=><option key={j.id} value={j.subject_name}>{j.subject_name}</option>)}</select>
          ...
          <button onClick={startAtt} style={{ width: '100%', padding: '15px', background: '#0891b2', color: '#fff', border: 'none', borderRadius: '10px', marginTop: '20px' }}>START</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{padding:'20px'}}>
      ...
      <div className="roll-grid">{students.map(s => (
        <div key={s.id} onClick={() => setMarked(p=>p.includes(s.id)?p.filter(x=>x!==s.id):[...p, s.id])} style={{ padding: '15px 5px', borderRadius: '10px', textAlign: 'center', background: marked.includes(s.id)?'#10b981':'#1e293b' }}>{s.id}</div>
      ))}</div>
      <button onClick={submitAtt} style={{ position: 'fixed', bottom: '20px', left: '20px', right: '20px', padding: '18px', borderRadius: '15px', background: '#10b981', color: '#fff', border: 'none', fontWeight: 'bold' }}>
        {loading ? 'SAVING...' : 'SUBMIT & DOWNLOAD'}
      </button>
    </div>
  );
        }
