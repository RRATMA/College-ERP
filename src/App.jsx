...
const injectStyles = () => {
  ...
  s.innerHTML = `
    ...
    .stat-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px; margin-bottom: 25px; }
    .stat-card { position: relative; overflow: hidden; border-left: 4px solid #06b6d4; padding: 15px; }
    .stat-card h3 { font-size: 24px; margin: 5px 0; font-weight: 800; color: #fff; }
    ...
    .tab-nav { display: flex; gap: 20px; margin-bottom: 20px; border-bottom: 1px solid #1e293b; overflow-x: auto; padding-bottom: 5px; }
    .tab-link { cursor: pointer; padding: 10px 5px; color: #64748b; font-weight: 700; font-size: 12px; white-space: nowrap; }
    .tab-link.active { color: #06b6d4; border-bottom: 2px solid #06b6d4; }
    .roll-btn { padding: 15px 0; border-radius: 10px; text-align: center; font-weight: 800; background: #1e293b; cursor: pointer; color: #94a3b8; border: 1px solid transparent; }
    .roll-btn.active { background: #10b981 !important; color: white !important; border-color: #34d399; }
    ...
    <div style={{ padding: '15px', maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div style={{display:'flex', alignItems:'center', gap:'10px'}}>
          ...
          <h2 style={{color:'#06b6d4', margin:0, fontSize:'18px'}}>HOD ADMIN</h2>
        </div>
        <LogOut onClick={() => setView('login')} color="#f43f5e" size={20} style={{cursor:'pointer'}}/>
      </div>

      <div className="stat-grid">
        <div className="glass stat-card"><h3>{db.l.length}</h3><p>Total Lects</p></div>
        <div className="glass stat-card"><h3>{theory}T | {practical}P</h3><p>Today's Spl</p></div>
        <div className="glass stat-card"><h3>{tLogs.reduce((a,c)=>a+c.present,0)}</h3><p>Today Pres</p></div>
        ...
        <div className="glass stat-card" style={{borderLeftColor:'#f43f5e'}}>
           <select id="dc" style={{fontSize:'11px', padding:'5px', marginBottom:'5px'}}>{sheets.map(s=><option key={s} value={s}>{s}</option>)}</select>
           <button onClick={()=>exportReport(document.getElementById('dc').value, true)} style={{padding:'5px', fontSize:'10px', background:'#f43f5e', color:'white', border:'none', borderRadius:'5px', width:'100%', fontWeight:800}}>DEFAULTER</button>
        </div>
      </div>
      ...
      {tab === 'staff' && (
        <div className="glass" style={{maxWidth:'400px'}}>
          <h3>Add Faculty</h3>
          <input id="fi" placeholder="ID"/><input id="fn" placeholder="Name"/><input id="fp" placeholder="Password"/>
      ...
      {tab === 'records' && (
        <div className="glass">
          {sheets.map(s => (
            <div key={s} style={{marginBottom:'10px', display:'flex', justifyContent:'space-between', alignItems:'center', background:'rgba(255,255,255,0.03)', padding:'10px', borderRadius:'10px'}}>
              <b>{s} Register</b>
              <button onClick={()=>exportReport(s, false)} className="btn-cyan" style={{width:'100px', padding:'8px', fontSize:'11px'}}>MASTER</button>
            </div>
          ))}
        </div>
      )}
...
// --- FACULTY PANEL (All Features Corrected) ---
function FacultyPanel({ user, setView }) {
  ...
  const exportFacultyMaster = async () => {
    ...
    try {
      ...
      ws.addRow([INSTITUTE_NAME]);
      ws.addRow([`SUB: ${setup.sub} | CLS: ${setup.cl}`]);
    ...
    <div style={{ padding: '20px', maxWidth: '500px', margin: '0 auto' }}>
      ...
      {setup.cl && (
        <div className="glass">
          ...
          <button className="btn-cyan" onClick={exportFacultyMaster} style={{marginTop:'10px', background:'#1e293b', border:'1px solid #06b6d4'}}>MY MASTER SHEET</button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ padding: '15px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(65px, 1fr))', gap: '8px', marginBottom:'100px' }}>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, padding: '20px', background: 'rgba(2,6,23,0.95)', borderTop:'1px solid rgba(255,255,255,0.1)' }}>
