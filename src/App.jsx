...
import { 
  ...
  User, Users, BarChart3, Plus, RefreshCw, BookOpen, Fingerprint, MapPin, CheckCircle2
} from 'lucide-react';
...
const RADIUS_LIMIT = 0.01; // Approx 500-800 meters
...
export default function App() {
  ...
  // १. स्टार्टअपला एक्सेल शीटची नावे लोड करणे
  useEffect(() => {
    fetch('/students_list.xlsx').then(res => res.arrayBuffer()).then(ab => {
      const wb = XLSX.read(ab, { type: 'array' });
      setExcelSheets(wb.SheetNames);
    }).catch(e => console.error("Excel mapping missing. Ensure students_list.xlsx is in public folder."));
  }, []);
  ...
  if (view === 'login') return (
    <div style={styles.loginPage}>
      <div style={styles.glassCard}>
        ...
        <p style={styles.badge}>INSTITUTIONAL MANAGEMENT</p>
  ...
  return (
    <div>
      <div className="grid-2" style={styles.statsRow}>
        ...
        <div style={styles.statBox}><CheckCircle2 color="#10b981"/> <div><small>Sessions</small><br/><b>{db.logs.length}</b></div></div>
      </div>
      ...
      {tab === 'logs' && (
        <>
          ...
          <button onClick={exportExcel} style={styles.downloadBtn}><Download size={18}/> DOWNLOAD MASTER EXCEL</button>
        ...
        return (
          <div key={f.id} style={styles.itemRow}>
            ...
            <div style={{display:'flex', gap:'8px', alignItems:'center'}}>
              <div style={styles.countTag}>T: {tCnt}</div>
              <div style={{...styles.countTag, color:'#a855f7'}}>P: {pCnt}</div>
              <button onClick={async ()=>{if(window.confirm("Delete?")){await supabase.from('faculties').delete().eq('id', f.id); loadAll();}}} style={{...styles.iconBtn, color:'#f43f5e'}}><Trash2 size={18}/></button>
            </div>
          </div>
        );
      })}
      ...
      {tab === 'manage' && (
        <div className="grid-2" style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px'}}>
          <div style={styles.formCard}>
            ...
            <button style={styles.btnAction} onClick={async ()=>{await supabase.from('faculties').insert([{id:form.i, name:form.n, password:form.p}]); loadAll(); alert("Done");}}>ADD FACULTY</button>
          </div>
          <div style={styles.formCard}>
            <h3><RefreshCw size={18}/> Assign Load</h3>
...
function FacultyPanel({ user }) {
  ...
  const startRollCall = () => {
    if(!setup.cl || !setup.start) return alert("Select all fields");
  ...
  const submitAttendance = () => {
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      if (dist > RADIUS_LIMIT) return alert("You are outside the campus radius!");
  ...
  if (!active) return (
    <div style={styles.setupCard}>
      ...
      <button style={styles.btnPrimary} onClick={startRollCall}>START ATTENDANCE</button>
    </div>
  );
...
const styles = {
  ...
  logoWrap: { background:'#fff', display:'inline-flex', padding:'12px', borderRadius:'15px', marginBottom:'20px' },
  title: { color:'#fff', margin:0, fontSize:'28px', fontWeight:'800' },
  ...
  userCircle: { width:'35px', height:'35px', background:'#6366f1', borderRadius:'10px', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:'bold' },
  ...
  statBox: { background:'rgba(30, 41, 59, 0.4)', padding:'20px', borderRadius:'20px', display:'flex', alignItems:'center', gap:'15px' },
  tabContainer: { display:'flex', background:'#0f172a', padding:'5px', borderRadius:'15px', marginBottom:'20px' },
  tabLink: { flex:1, border:'none', color:'#fff', padding:'10px', borderRadius:'12px', cursor:'pointer' },
  ...
  formCard: { background:'rgba(30, 41, 59, 0.4)', padding:'20px', borderRadius:'20px', border:'1px solid rgba(255,255,255,0.05)' },
  ...
  stickyHeader: { display:'flex', justifyContent:'space-between', alignItems:'center', padding:'15px', background:'#1e293b', borderRadius:'15px', marginBottom:'20px' },
  backBtn: { background:'none', border:'none', color:'#fff', cursor:'pointer' },
  downloadBtn: { width:'100%', background:'#10b981', color:'white', padding:'15px', borderRadius:'12px', border:'none', cursor:'pointer', marginBottom:'15px' }
};
